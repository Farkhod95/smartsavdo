...
TG_SECRET = os.getenv("change-me-strong-secret", "")
...
def unwrap_list(data: JSONType) -> List[Dict[str, Any]]:
    """
    Supports DRF pagination or plain list:
      - [{"id":..,"name":..}, ...]
      - {"count":..., "results":[...]}
      - {"results":[...]} (custom)
    """
...
async def api_post(session: aiohttp.ClientSession, path: str, payload: dict) -> Dict[str, Any]:
    url = f"{API_BASE}{path}"
    headers = {"X-TG-SECRET": TG_SECRET, "Content-Type": "application/json"}
...
async def build_regions_keyboard() -> InlineKeyboardMarkup:
    async with aiohttp.ClientSession() as session:
        data = await api_get(session, "/region/public/")
    regions = unwrap_list(data)

    buttons = [
        InlineKeyboardButton(text=pick_name(r), callback_data=f"region:{r.get('id')}")
        for r in regions
...
async def build_districts_keyboard(region_id: int) -> InlineKeyboardMarkup:
    # District public list should support ?region_id=...
    async with aiohttp.ClientSession() as session:
        data = await api_get(session, "/district/public/", params={"region_id": region_id})
    districts = unwrap_list(data)

    buttons = [
        InlineKeyboardButton(text=pick_name(d), callback_data=f"district:{d.get('id')}")
        for d in districts
...
async def main() -> None:
    ...
    async def handle_full_name(message: Message, state: FSMContext):
        ...
        try:
            ...
            await message.answer(f"Regionlarni olishda xatolik.\n{e}")
            return
    ...
    async def handle_region(call: CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            await call.message.answer(f"Tumanlarni olishda xatolik.\n{e}")
            await call.answer()
            return
    ...
    async def handle_get_token(call: CallbackQuery):
        ...
        if not access or not refresh:
            await call.message.answer(f"❌ Noto‘g‘ri javob: {tok}")
            await call.answer()
            return
